		...
		if (enet_host_service(host, &event, 1) > 0)
		{
			do
			{
				...
				case ENET_EVENT_TYPE_RECEIVE:
				{
					...
					// On g√®re le message qu'on a re√ßu
...
void UnserializeMap(std::string mapPath) {
	...
	//std::cout << "DonnÈes JSON chargÈes: " << data.dump(4) << std::endl; // Affichage de maniËre lisible

	MapData mapData;
	mapData.from_json(data);

	//// Optionnel: Affiche les objets physiques chargÈs
	//for (const auto& obj : mapData.physicObjects) {
	//	std::cout << "Objet chargÈ de type: " << obj->Type << std::endl;
